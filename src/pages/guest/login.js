...
const LoginPage = () => {
    const { isFormValid, showPassword, formData, errorMessages, handleInputChange, toggleShowPassword, setFormData, setErrorMessages, setIsFormValid } = useForm();
    ...
    const handleSubmit = (e) => {
        ...
        if (isFormValid) {
            // Lakukan pengiriman data atau aksi lainnya setelah validasi sukses
            router.push('/')
            ...
            <div className="h-full mt-10 mb-6 md:flex">
                ...
                <div className={`flex md:w-1/2 shadow-3xl justify-center py-10 items-center border-l-4 border-dashed ${bgColor} rounded-lg`}>
                    <form className={bgColor} onSubmit={handleSubmit}>
                        ...
                        <button type="submit" disabled={!isFormValid} className="block w-full bg-indigo-600 mt-2 py-2 rounded-2xl text-white font-semibold mb-2">Login</button>
                        <span className="text-sm ml-2">Don&apos;t have an account? <Link href={'/guest/register'} className='hover:text-blue-500 cursor-pointer'>Sign up</Link></span>
                    </form>
                </div>
            </div>
            <SponsorComp />
